...
rest_locations_new = rest_locations %>%
  ...
  drop_na(zipcode) %>% 
  replace(is.na(.), 0) %>% 
  mutate(total = beverages_dessert + `fast food` + `sit-down`, 
         prop_bev = beverages_dessert/total, 
         prop_fast = `fast food`/total, 
         prop_sit = `sit-down`/total, 
         logit_sit = ifelse(`sit-down` > 0, 1, 0)) %>% 
  select(-c(total)) 
...
fit <- lm(prop_fast ~ prop_white + prop_bipoc + prop_female + prop_other + med_income, data = total_rest_acs)
...
fit.logit <- glm(logit_sit ~ prop_white + prop_bipoc + prop_female + prop_other + med_income, data = total_rest_acs, family = "binomial")

summary(fit.logit)

logit_table = fit.logit %>% 
  broom::tidy() %>% 
    knitr::kable(digits = 3)
```
...
ggplot(total_rest_acs, aes(x = prop_female, y = prop_fast)) +
  ...
  labs(
    title = "Median income by total population",
    x = "Total population",
    y = "Median income"
  )
